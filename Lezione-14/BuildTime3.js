...
function searchJob() {
  ...
  getResult(position, location);
}


function getResult(position, location) {
  results = [];
  position = position.toUpperCase();
  location = location.toUpperCase();
  for (job of jobs) {
    elemPosition = job.title.toUpperCase();
    elemLocation = job.location.toUpperCase();
    if (elemPosition.indexOf(position) > -1 && elemLocation.indexOf(location) > -1) {
      let item = { 'position': elemPosition, 'location': elemLocation }
      results.push(item);
    }
  }
  console.log(results);
  ...
  for (result of results) {
    ...
    // let jobDescription =  document.createTextNode(result.position+" "+result.location);
    // let item = li.appendChild(jobDescription);
    li.innerHTML = result.position + " " + result.location;
